...
- (UIScrollView *)segmentTitleView {
    if (!_segmentTitleView) {
        ...
        _segmentTitleView.backgroundColor = SegmentBgColor;
        _segmentTitleView.showsHorizontalScrollIndicator = NO;
        _segmentTitleView.showsVerticalScrollIndicator = NO;
        self.button_W = (_size.width - Space_W * (_titleArr.count -1)) / _titleArr.count;
...
- (UIView *)indicateView {
    if (!_indicateView) {
        ...
        _indicateView.backgroundColor = lineTintColor;
...
- (void)loadSegmentTitleViewWithSegmentStyle:(JHSegementStyle)style {
    ...
    if (style == YHSegementStyleIndicate) {
        ...
        for (int i = 0; i < _titleArr.count; i++) {
            ...
            button.frame = CGRectMake(item_x, 0, _buttonSpace *2 + titleSize.width, SegmentViewHeight);
            [button setTag:i];
            ...
            if (i == 0) {
                ...
                self.indicateView.frame = CGRectMake(_buttonSpace, SegmentViewHeight - 6, titleSize.width, IndicateHeight);
        ...
        for (int i = 0; i < _titleArr.count; i++) {
            title = _titleArr[i];
            UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
            button.frame = CGRectMake(item_x, 0, self.button_W ,SegmentViewHeight);
            [button setTag:i];
            if (i != _titleArr.count - 1) {
                UIView *spaceLine = [[UIView alloc] init];
                spaceLine.frame = CGRectMake(self.button_W + (self.button_W + Space_W)*i , 6, Space_W, SegmentViewHeight - 12);
            ...
            [self.widthBtnArr addObject:[NSNumber numberWithDouble:CGRectGetWidth(button.frame)]];
            item_x += self.button_W + Space_W;
            if (i == _titleArr.count - 1) {
                item_x = item_x - Space_W;
            }
...
- (void)didClickButton:(UIButton *)button {
    ...
    if (_jh_resultBlock) {
        _jh_resultBlock(_selectedButton.tag);
...
- (void)scrollIndicateView {
    ...
    [UIView animateWithDuration:Duration_time delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
...
- (void)setYh_indicateStyle:(JHSegementIndicateStyle)yh_indicateStyle {
    _yh_indicateStyle = yh_indicateStyle;
    if (yh_indicateStyle == YHSegementIndicateStyleDefault) {
        
    }else {
        _indicateView.frame = CGRectMake(_selectedButton.frame.origin.x, SegmentViewHeight - IndicateHeight, [self widthAtIndex:0], IndicateHeight);
    }
}
